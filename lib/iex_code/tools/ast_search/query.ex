defmodule IexCode.Tools.ASTSearch.Query do
  ...
  def filter(symbols, _), do: symbols
  ...
  defp matches_name?(s, name) when is_binary(name) do
    String.downcase(s.name) == String.downcase(name) or
      String.contains?(String.downcase(s.name), String.downcase(name))
  end

  defp matches_name?(s, name) when is_atom(name), do: matches_name?(s, to_string(name))
  defp matches_name?(s, %Regex{} = regex), do: Regex.match?(regex, s.name)

  defp matches_module?(_s, nil), do: true

  defp matches_module?(s, mod) when is_binary(mod) do
    mod_str = String.downcase(mod)
    target_mod = String.downcase(s.module || if(s.type == :module, do: s.name, else: ""))
  ...
  defp matches_module?(s, %Regex{} = regex) do
    target_mod = s.module || if s.type == :module, do: s.name, else: ""
    Regex.match?(regex, target_mod)
  end
  ...
  defp matches_visibility?(_s, _), do: true
  ...
  defp matches_path?(s, path) when is_binary(path) do
  ...
  defp matches_text?(s, text) when is_binary(text) do
    ...
    String.contains?(String.downcase(s.code || ""), t_down) or
      String.contains?(String.downcase(s.name), t_down)
  end
