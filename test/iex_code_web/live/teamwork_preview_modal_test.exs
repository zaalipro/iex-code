defmodule IexCodeWeb.Live.TeamworkPreviewModalTest do
  use IexCode.E2E.Case, async: false
  @moduletag mock_llm: true

  describe "Teamwork Preview Modal & Orchestration Controls" do
    test "opens and closes teamwork preview modal via events and buttons", %{
      conn: conn,
      workspace_path: path
    } do
      ...
      refute has_element?(view, "#teamwork-preview-modal")

      # Open modal via toolbar button or event
      render_click(view, "open_teamwork_preview")
      assert has_element?(view, "#teamwork-preview-modal")
      ...
      render_click(view, "close_teamwork_preview")
      refute has_element?(view, "#teamwork-preview-modal")
    end
    ...
    test "composer parses and routes compound /teamwork-preview /boost /goal command", %{
      conn: conn,
      workspace_path: path
    } do
      project = create_project_fixture(%{root_path: path})
      ...
      render_submit(view, "submit_prompt", %{
        "prompt" => "/teamwork-preview /boost /goal Synthesize high-throughput streaming pipeline"
      })
    ...
    test "RunComponents renders teamwork blueprint and boost telemetry cards on run overview" do
      blueprint =
        IexCode.Execution.TeamworkPreview.generate_blueprint(
      ...
      run = %IexCode.Runs.Run{
        ...
        metadata: %{
          "boost" => true,
          "teamwork_blueprint" => IexCode.Execution.TeamworkPreview.to_map(blueprint)
        }
      }
